...
class Args:
    ...
    def __init__(self):
        self.batch_size = 4  # Reduced for global scale
        self.gpu_num = 1
        self.img_size = 64  # May need adjustment for global scale
        self.num_layers = 1  # Start with 1 layer for global scale
        self.frame_num = 3  # Sequence length
        self.input_dim = 1  # single channel for microplastics concentration
        self.hidden_dim = 32  # May need adjustment for global scale
        self.learning_rate = 0.001
        self.epochs = 3  # Start with fewer epochs for global scale
        self.patch_size = 4  # Patch size for attention
...
def downsample_data(data_array, target_height=64, target_width=64):
    """Downsample to target resolution for global coverage."""
...
def process_and_plot_to_array(nc_file):
    """Convert a netCDF file to a global normalized array."""
    ds = xr.open_dataset(nc_file)
    data = ds['mp_concentration']
    data_array = data.values
    data_array_2d = data_array.squeeze()
    
    # Use the full global dataset - no regional cropping
    print(f"Global data shape: {data_array_2d.shape}")
...
def process_and_plot_to_array_raw(nc_file):
    """Convert a netCDF file to a global array WITHOUT normalization."""
    ds = xr.open_dataset(nc_file)
    data = ds['mp_concentration']
    data_array = data.values
    data_array_2d = data_array.squeeze()
    ...
    if nc_file == nc_files[0]:
        print(f"Global raw data shape: {data_array_2d.shape}")
    
    # Return raw data WITHOUT normalization
    return data_array_2d

def extract_data_and_clusters(nc_files):
    """Extract and preprocess global data for training with GLOBAL normalization."""
    data = []
    ...
    data = np.array(data)
    print(f"\nFinal global data shape: {data.shape}")
    print(f"Geographic region: Global (full Earth coverage)")
...
def evaluate_sa_convlstm(model, test_loader, timestamps, test_start_idx):
    ...
    # Global geographic bounds for proper visualization
    ...
    for i in range(num_samples):
        ...
        if test_idx < len(nc_files):
            ...
            downsampled_raw = downsample_data(raw_data)
            raw_targets.append(downsampled_raw)
        else:
    ...
    # Ground truth colorbar (Raw Scale)
    ...
    plt.tight_layout()
    plt.savefig('sa_convlstm_global_comprehensive_evaluation.png', dpi=150, bbox_inches='tight')
    
    # Log the comprehensive visualization to wandb
    wandb.log({"comprehensive_evaluation": wandb.Image('sa_convlstm_global_comprehensive_evaluation.png')})
    ...
    for i in range(min(6, len(predictions))):  # Detailed view for first 6 samples
        ...
        date_str = timestamps[test_start_idx + i].strftime("%Y-%m-%d")
        fig.suptitle(f'Detailed Global Comparison: {date_str} (Sample {i+1})', fontsize=16)
...
SSIM: {ssim_scores[i]:.4f}"""
        ...
        plt.tight_layout()
        comparison_path = f'detailed_global_comparison_{i+1}.png'
    ...
    np.save('test_predictions_global.npy', predictions)
    np.save('test_targets_global.npy', targets)
    np.save('test_timestamps_global.npy', np.array([timestamps[test_start_idx + i] for i in range(len(predictions))]))
    ...
    with open('sa_convlstm_global_evaluation_results.json', 'w') as f:
        json.dump(evaluation_results, f, indent=2)
    
    print(f"\n✓ Saved test predictions to: test_predictions_global.npy")
    print(f"✓ Saved test targets to: test_targets_global.npy")
    print(f"✓ Saved evaluation results to: sa_convlstm_global_evaluation_results.json")
    print(f"✓ Created comprehensive visualization: sa_convlstm_global_comprehensive_evaluation.png")
...
def save_forecasts_as_netcdf(forecasts, forecast_dates, args):
    ...
    forecast_dir = Path("forecast_netcdf_global")
    ...
    # Create coordinate arrays for the downsampled global grid
    ...
    for i, (forecast, date) in enumerate(zip(forecasts, forecast_dates)):
        ...
        filename = f"sa_convlstm_forecast.s{date_str}-120000-e{date_str}-120000.l3.grid-microplastic.global.nc"
        ...
        ds = xr.Dataset(
            data_vars={
                'mp_concentration': (
                    ...
                    forecast_geo,
                    {
                        'long_name': 'Global Microplastic Concentration Forecast',
            ...
            attrs={
                'title': 'SA-ConvLSTM Global Microplastic Concentration Forecast',
                ...
                'lat_bounds': f'{lat_min:.1f}N to {lat_max:.1f}N',
                'lon_bounds': f'{lon_min:.1f}E to {lon_max:.1f}E',
...
def forecast_future(model, data, timestamps, args, num_forecast_days=5):
    ...
    fig.suptitle('SA-ConvLSTM 5-Day Global Microplastics Forecast\n(Global Future Predictions)', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig('sa_convlstm_global_forecast.png', dpi=150, bbox_inches='tight')
    
    # Log forecast visualization to wandb
    wandb.log({"forecast_visualization": wandb.Image('sa_convlstm_global_forecast.png')})
    ...
    for i in range(num_forecast_days):
        ...
        ax.set_title(f'Global Microplastics Forecast\nDay +{i+1}: {forecast_dates[i].strftime("%Y-%m-%d")}')
        ax.axis('off')
        plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        
        plt.tight_layout()
        forecast_path = f'forecast_global_day_{i+1}.png'
...
if __name__ == "__main__":
    print("=" * 60)
    print("SA-CONVLSTM WITH MICROPLASTICS DATA - GLOBAL")
    print("=" * 60)
    ...
    # Initialize wandb
    wandb.init(
        project="sa-convlstm-microplastics-global",
        name=f"sa-convlstm-global-{args.img_size}x{args.img_size}-h{args.hidden_dim}-l{args.num_layers}",
        config={
            "model_type": "SA-ConvLSTM Global",
            "framework": "PyTorch",
    ...
    plt.tight_layout()
    plt.savefig('sa_convlstm_global_training.png', dpi=150, bbox_inches='tight')
    
    # Log training history to wandb
    wandb.log({"training_history": wandb.Image('sa_convlstm_global_training.png')})
    
    plt.close()
    
    # Save model
    torch.save(model.state_dict(), 'models/sa_convlstm_global_microplastics.pth')
    
    # Save results
    results = {
        'model_type': 'SA-ConvLSTM Global with Self-Attention',
        'framework': 'PyTorch',
        'device': str(device),
        'sequence_length': args.frame_num,
        'image_size': args.img_size,
        ...
        'geographic_region': {
            ...
            'lat_bounds': [-90, 90],
            'lon_bounds': [-180, 180],
            'description': 'SA-ConvLSTM model with self-attention for global microplastics prediction'
        }
    }
    
    with open('sa_convlstm_global_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print("\n" + "=" * 60)
    print("SA-CONVLSTM GLOBAL TRAINING COMPLETED")
    print("=" * 60)
    print(f"✓ Test MAE: {mae:.6f}")
    print(f"✓ Mean SSIM: {mean_ssim:.4f}")
    ...
    print("\nFiles generated:")
    print("- sa_convlstm_global_comprehensive_evaluation.png")
    print("- sa_convlstm_global_training.png") 
    print("- sa_convlstm_global_results.json")
    print("- models/sa_convlstm_global_microplastics.pth")
    print("=" * 60)
    
    # FORECASTING: Retrain on full dataset and predict future
    print("\n" + "=" * 60)
    print("STARTING GLOBAL FORECASTING PHASE")
    print("=" * 60)
    ...
    # Save forecast model
    torch.save(forecast_model.state_dict(), 'models/sa_convlstm_global_forecast_model.pth')
    
    # Save forecast results
    forecast_results = {
        'forecast_type': 'SA-ConvLSTM 5-Day Global Future Prediction',
        ...
        'geographic_region': {
            'name': 'Global',
            'description': 'Future microplastics predictions for entire Earth'
        }
    }
    
    with open('sa_convlstm_global_forecast_results.json', 'w') as f:
        json.dump(forecast_results, f, indent=2)
    
    print("\n" + "=" * 60)
    print("GLOBAL FORECASTING COMPLETED")
    print("=" * 60)
    print(f"✓ Generated 5-day global forecast from {timestamps[-1].strftime('%Y-%m-%d')}")
    ...
    print("- sa_convlstm_global_forecast.png")
    print("- forecast_global_day_1.png to forecast_global_day_5.png")
    print("- sa_convlstm_global_forecast_results.json")
    print("- models/sa_convlstm_global_forecast_model.pth")
    print("=" * 60)
